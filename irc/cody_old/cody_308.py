...
class Cody():  
	...
	def codyCompliment(self, trigger):
		...
		if len(self.MSG_BODY) > len(trigger + '\r\n'):
			...
			complimentmakersite = "http://complimentmaker.com/index.php?user="
			name 				= self.MSG_BODY.split()[1]
			...
			self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+" :" +compliment[0]+ "\r\n")
	...
	def codyInsult(self, trigger):
		...
		if len(self.MSG_BODY) > len(trigger + '\r\n'):
			...
			complimentmakersite = "http://hatemaker.com/index.php?user="
			name 				= self.MSG_BODY.split()[1]
			compsite 			= complimentmakersite+name
			compsitereq			= Request(compsite)
			compresult 			= urlopen(compsitereq)
			...
			self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+" :" +insult[0]+ "\r\n")
	...
	def codySlogan(self, trigger):
		...
		if len(self.MSG_BODY) > len(trigger + '\r\n'):
			...
			complimentmakersite = "http://www.sloganmaker.com/sloganmaker.php?user="
			name 				= self.MSG_BODY.split()[1]
			compsite 			= complimentmakersite+name
			compsitereq			= Request(compsite)
			compresult 			= urlopen(compsitereq)
			...
			self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+" :" +slogan[0]+ "\r\n")
