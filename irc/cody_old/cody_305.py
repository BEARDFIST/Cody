...
!
!
!!!!	CODYCONVERT NO LONGER WORKS AS OF NOVEMBER 1ST, GOOGLE HAS CLOSED iGOOGLE.
!
!
!
!		
		...
		if len(self.MSG_BODY) > len('!cody.kickstarter\r\n'):
			if "/projects/" in self.MSG_BODY:
				 ...
				 if len(projectToAddsplit) != 3:
				 	...
				 	kickfile.close()
				 	self.IRC.send('NOTICE '+self.MSG_NICK+' '+self.MSG_CHANNEL+' :Kickstarter project has been added' )
	...
	#the twitter function will be kept in for documenting how to use the twitter API, but this code will no longer be used.
	"""
	def codyTwitter(self, trigger):

		#TURN OFF FEEDS
		if '!cody.Twitter' in trigger \
		and self.POST_twitz_FEEDS:
			self.POST_twitz_FEEDS = False
			self.IRC.send('PRIVMSG '+self.MSG_CHANNEL+' :twitter feeds are now turned off.\r\n')

		#TURN ON FEEDS
		elif '!cody.Twitter' in trigger \
		and not self.POST_twitz_FEEDS:
			self.POST_twitz_FEEDS = True
			self.IRC.send('PRIVMSG '+self.MSG_CHANNEL+' :twitter feeds are now turned on.\r\n')

	    #CHECK FOR NEW DATA 4 TIMES AN HOUR
		elif 'codyTwitter' in trigger \
		and (datetime.datetime.now().minute % 20) == 0 \
		or '!cody.tweetme' in trigger:
			
			#FETCH THE TWEETS AND PREPARE DATA FOR POSTING
			following = (self.TWITTER_FEEDS)
			
			people = following.split(",")

			#TWITTER PROVIDED IDENTIFIERS TO RECOGNIZE PERMISSION SETTINGS FOR THE ACCOUNT
			CONSUMER_KEY = "R6s0KOotjoilqVcMo8ZFcA"
			CONSUMER_SECRET = "FPnWrJ1kovDg3AnFeNHYvGVa5ugnZxCiXDzKeZ4MkU"

			#A FILE CONTAINING HASHED LOGIN VALUES
			MY_TWITTER_CREDS = "resources/.twitterbot_oauth"

			try:
				#IF NO AUTH FOUND, IT WILL TRY TO CREATE A NEW AUTH (THIS WILL BREAK THE LOOP)
				if not os.path.exists(MY_TWITTER_CREDS):
				    oauth_dance("1nverTwitz", CONSUMER_KEY, CONSUMER_SECRET, MY_TWITTER_CREDS)

				oauth_token, oauth_secret = read_token_file(MY_TWITTER_CREDS)

				#CREATES A SESSION AS A PYTHON OBJECT TO TWITTER IN WHICH DATA IS EXCHANGED
				twitter = Twitter(auth=OAuth(oauth_token, oauth_secret, CONSUMER_KEY, CONSUMER_SECRET))

				#PREP TWEETS FOR IRC
				tweets = []
				for person in people:
					tweets += twitter.statuses.user_timeline(id=person)

				tweetout = []
				for tweet in tweets:
					#bold chr(0x02)
					#regular chr(0x0f)
					tweetout.append(tweet['user']['screen_name'] + " tweeted: "+ tweet['text'] + " https://twitter.com/" + tweet['user']['screen_name'])


				checkfile = open("resources/twitterFeeds.db", "r").read()
				newData = []

				for item in tweetout:
					newDataItem = item.encode('ascii','ignore')

					if newDataItem not in checkfile:
						newData.append(newDataItem)

				if newData:

						#REMAKE THE FILE IF THE FILE NOW EXCEEDS 500 LINES
					if len(checkfile.splitlines()) > 2000:
						with open("resources/twitterFeeds.db", "w") as feedFile:
							for twat in tweetout:
								feedFile.write('\n')
								feedFile.write(twat.encode('ascii','ignore'))
							feedFile.close()

					#IF NOT, JUST APPEND TO IT
					else:
						with open("resources/twitterFeeds.db", "a") as feedfile:
							for item in newData:
								feedfile.write('\n')
								feedfile.write(item.encode('ascii','ignore') + '\r\n')

							feedfile.close()

				#PRINT NEW DATA TO CHANNEL
				for item in reversed(newData):
					thetweettopost = item.encode('ascii','ignore')
					tweetsplit = thetweettopost.partition(" tweeted: ")
					#bold chr(0x02) and regular chr(0x0f)
					fulltweet = chr(0x02) + tweetsplit[0] + tweetsplit[1] + chr(0x0f) + tweetsplit[2]
					
					self.IRC.send('PRIVMSG '+self.TWITTER_FEED_CHANNELS+' :' + fulltweet + '\r\n' )
					time.sleep(1.5)
			except Exception, e:
				self.IRC.send('PRIVMSG '+self.TWITTER_FEED_CHANNELS+' :' + "Something went wrong with the twitter function!" + '\r\n' )
