...
class Cody():  
	...
	def __init__(self):
		...
		self.PUBLIC_FUNCTIONS			=	{	 '!cody.python':self.codyPython,		' cody':self.hiCody,					'!cody.version':self.codyVersion, 				 
												 ...
												 '!cody.kickstarter':self.kickstarter }
	...
	def kickstarter(self, trigger):
		...
		def IntCutter(fullstring, starter, ender):
			...
			stringint		= stringsplit[0]
			return			int(stringint)


		req 				= Request('http://www.kickstarter.com/projects/sistratton/banshee-the-game-and-feature-film/comments')
		req.add_header		('User-agent', 'Mozilla 5.10')
		result 				= urlopen(req)
		html 				= result.read()

		people 				= IntCutter(html, 'itemprop="Project[backers_count]" value="', '">' )
		pledge 				= IntCutter(html, 'Project[pledged]" value="', '">')
		daysleft 			= IntCutter(html, '<meta property="twitter:data2" content="', '"/>')

		goal 				= 14500
		togo 				= goal - pledge
		average				= pledge / people
		neededPerPerson		= togo / average

		self.IRC.send('PRIVMSG '+self.MSG_CHANNEL+' :Banshee Kickstarter pledges so far: '							+str(pledge)+
													' GBP. - Left to go: '											+str(togo)+
													' GBP. - Backers: '												+str(people)+
													' - Average per backer: '										+str(average)+
													' GBP. - Additional backers needed pledging average amount: '	+str(neededPerPerson)+
													' people. - Days left: '										+str(daysleft)+ '/30'
																													+'\r\n' )
