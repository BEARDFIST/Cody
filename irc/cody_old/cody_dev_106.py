#!/usr/bin/python
import socket, time, os, sys, stat, inspect, random, datetime, re
...
#!cody.mail
	...
	elif	'!cody.mail' 		in MESSAGE.lower()  \
	and 	connected 			== True 			\
	and 	authenticateNick() 	== False:

		irc.send('PRIVMSG '+CHANNEL+' :Sorry '+NICK+', you don\'t have permission to do that.\r\n' )	

#!cody.convert
	if     ' in ' 		in MESSAGE 	\
	and 	connected 	== True	    :

		findAmount 		= re.search(r'\d+', MESSAGE)
		findInputUnit  	= re.search(r'[a-z]+', MESSAGE.lower())
		findOutputUnit	= re.search(r' in ([a-z]+)', MESSAGE.lower())
		...
		and findOutputUnit :
			...
			#fetch data from MESSAGE to be sent to google
			inputAmount  =  findAmount.group()
			inputUnit 	 =  findInputUnit.group()
			outputUnit 	 =  findOutputUnit.group(1)

			#check if there are any spaces in our data and, if so, convert them to %20 so we can use them in a URL
			if ' ' in inputAmount:
			...
			if 'error: ""' in convertedAmount:
				...
				irc.send('PRIVMSG '+CHANNEL+' :'+inputAmount+' '+inputUnit+' = '+convertedAmount+'\r\n' )

			elif 'error: "4"' in convertedAmount:
				irc.send('PRIVMSG '+CHANNEL+' :Request failed: unit not recognized \r\n' )			

			elif 'error: "Unit mismatch"' in convertedAmount:
				irc.send('PRIVMSG '+CHANNEL+' :Request failed: units are not related \r\n' )
