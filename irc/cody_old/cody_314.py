...
class Cody():  
	...
	def __init__(self):
		...
		self.ADMIN_FUNCTIONS 			= 	{	 '!cody.join':self.joinChannel,			'!cody.part':self.partChannel, 			'!cody.reload':self.codyReload, 	
												 ...
												 '!cody.portscan':self.portscan, 		'!cody.diag':self.codyDiag }
	...
	def codyDiag(self, trigger):
		...
		def cpuUsage():
			try:
				cmd 	= ["top -b -d1 -n1|grep -i 'Cpu(s)'|head -c21|cut -d ' ' -f3|cut -d '%' -f1"] #linux magic
				pro 	= Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
				output 	= pro.communicate()
				splitit	= str(output[0])
				output 	= splitit.split()
			except:
				pass
		...
		def sys_uptime():
			try:
				cmd 	= ["uptime | cut -d ' ' -f 4"] #linux magic uptime
				pro 	= Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
				output 	= pro.communicate()
				splitit	= str(output[0])
				output 	= splitit.split()
		...
		def motdinfo(): #by far the worst code I've ever written, you're very welcome to revise!
			try:
				values = []
				#path = 'E:/Dropbox/Cody/Concepts/motd.txt'
				path = '/etc/motd'
				with open(path, 'r') as motdfile:
				...
				updates = int(output[3])+int(output[4])
				output = [output[0], updates]
