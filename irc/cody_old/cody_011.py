...
#Parsing console (from linux) chat data	
	if(connected == True and ('Inveracity' in data or 'lmn' in data or 'Stoy' in data)):
...
#QUIT
	if data.find ( '!cody quit' ) != -1 or data.find ( '!cody fuck off' ) != -1 or data.find ( '!cody beat it' ) != -1 or data.find ( '!cody gtfo' ) != -1 and connected == True:
		...
		if(reloaderNick == 'lmn' or reloaderNick == 'Inveracity' or reloaderNick == 'Stoy'):
...
#hiCody	
	...
	if data.lower().find ('hi' ) != -1 			\
	or data.lower().find ('hello') != -1 		\
	or data.lower().find ('sup')	!= -1 		\
	or data.lower().find ('yo' ) != -1 			\
	or data.lower().find ('hey' ) != -1 		\
	or data.lower().find ('hola') != -1 		\
	or data.lower().find ('you suck' ) != -1 	\
	or data.lower().find ('i love you') != -1 	\
	and connected == True  						:				
		...
		or MESSAGE.lower().find('codes') != -1:
			...
			or NICK == 'Inveracity'				\
			or NICK == 'Stoy'					:			
				irc.send ( 'PRIVMSG #code :'+random.choice(hiCodyReplies)+'\r\n' )
...
#admin greeting
	...
	if connected == True and NICK == 'Inveracity' and 'JOIN #code' in data:
		irc.send ( 'PRIVMSG #code :'+random.choice(InveracityMessages) +'\r\n' )
        
	if connected == True and NICK == 'lmn' and 'JOIN #code' in data:
		irc.send ( 'PRIVMSG #code :'+random.choice(lmnMessages)+'\r\n' )
                
	if connected == True and NICK == 'Stoy' and 'JOIN #code' in data :
		irc.send ( 'PRIVMSG #code :'+random.choice(StoyMessages)+'\r\n' )
