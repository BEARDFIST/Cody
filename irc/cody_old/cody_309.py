...
class Cody():  
	...
	def __init__(self):
		...
		self.PUBLIC_FUNCTIONS			=	{	 '!cody.python':self.codyPython,		' cody':self.hiCody,					'!cody.version':self.codyVersion, 				 
												 ...
												 '!cody.compliment':self.codyCompliment,'!cody.insult':self.codyInsult,			'!cody.slogan':self.codySlogan }
	...
	def codyCompliment(self, trigger):
		self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+ " :Cody got banned for overusage\r\n")
	def codyInsult(self, trigger):
		self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+ " :Cody got banned for overusage\r\n")
	def codySlogan(self, trigger):
		self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+ " :Cody got banned for overusage\r\n")
...
"""
	def codyCompliment(self, trigger):
		...
		if len(self.MSG_BODY) > len(trigger + '\r\n'):
			...
			try:
				inputstring 		= self.MSG_BODY.lower().split("!cody.compliment ")
				name 				= {'user':inputstring[1]}
				name 				= urlencode(name)
				complimentmakersite = "http://complimentmaker.com/index.php?"
				compsite 			= complimentmakersite+name
				compsitereq			= Request(compsite)
				compresult 			= urlopen(compsitereq)
				html 				= compresult.read()
				htmlsplit1 			= html.partition('<div align="center"><p>')
				compliment 			= htmlsplit1[2].partition('</p></div> <form name="FormName"')

				print compliment

				if '\r\n' in compliment[0]:
					compliment = compliment[0].replace('\r\n', '')
					print "return newline found and replaced"
					print compliment

				else:
					compliment = compliment[0]

				self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+" :" +compliment+ "\r\n")

			except Exception as e:
				print e
				self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+" :An error occured, try again later.\r\n")

		else:
			self.IRC.send("NOTICE "+self.MSG_NICK+' '+self.MSG_CHANNEL+" :Usage: !cody.compliment <name of person>\r\n")
	...
	def codyInsult(self, trigger):
		...
		if len(self.MSG_BODY) > len(trigger + '\r\n'):
			...
			try:
				inputstring 		= self.MSG_BODY.lower().split("!cody.insult ")
				name 				= {'user':inputstring[1]}
				name 				= urlencode(name)
				complimentmakersite = "http://hatemaker.com/index.php?"
				compsite 			= complimentmakersite+name
				compsitereq			= Request(compsite)
				compresult 			= urlopen(compsitereq)
				html 				= compresult.read()
				htmlsplit1 			= html.partition('<div align="center"><p>')
				insult 				= htmlsplit1[2].partition('</p></div> <form name="FormName"')

				print insult

				if '\r\n' in insult[0]:
					insult = insult[0].replace('\r\n', '')
					print "return newline found and replaced"
					print insult
				...
				self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+" :" +insult+ "\r\n")
			...
			self.IRC.send("NOTICE "+self.MSG_NICK+' '+self.MSG_CHANNEL+" :Usage: !cody.insult <name of person>\r\n")
	...
	def codySlogan(self, trigger):
		...
		if len(self.MSG_BODY) > len(trigger + '\r\n'):
			...
			try:
				inputstring 		= self.MSG_BODY.lower().split("!cody.slogan ")
				name 				= {'user':inputstring[1]}
				encodedname 		= urlencode(name)
				complimentmakersite = "http://sloganmaker.com/sloganmaker.php?" 
				compsite 			= complimentmakersite+encodedname
				compsitereq			= Request(compsite)
				compresult 			= urlopen(compsitereq)
				html 				= compresult.read()
				htmlsplit1 			= html.partition('<div align="center"><p>')
				slogan 				= htmlsplit1[2].partition('</p></div>    <form name="FormName"')

				print slogan
				if '\r\n' in slogan[0]:
					slogan = slogan[0].replace('\r\n', '')
					print "return newline found and replaced"
					print slogan
				...
				self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+" :" +slogan+ "\r\n")
			...
			self.IRC.send("NOTICE "+self.MSG_NICK+' '+self.MSG_CHANNEL+" :Usage: !cody.slogan <name of person>\r\n")
"""
