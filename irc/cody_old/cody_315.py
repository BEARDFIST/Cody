...
class Cody():  
	...
	def codyDiag(self, trigger):
		...
		def cpuUsage():
			try:
				cmd 	= ["top -b -d1 -n1|grep -i 'Cpu(s)'|head -c21|cut -d ' ' -f3|cut -d '%' -f1"] #linux magic: NEW LINUX MAGIC: top -bn2 | grep "Cpu(s)" | sed "s/.*, *\([0-9.]*\)%* id.*/\1/" | awk '{print 100 - $1"%"}'
		...
		def sys_uptime():
			try:
				cmd 	= ["uptime | cut -d ' ' -f 5"] #linux magic uptime
				pro 	= Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
				output 	= pro.communicate()
				splitit	= str(output[0])
				output 	= splitit.split()
		...
		if diskStats and cpuStats:
			...
			self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+ " :"+ \
														...
														reset + "| System uptime: " 	+ orange +str(sysUptime)+ " days "+\
														"\r\n")
		else:
