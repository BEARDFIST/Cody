	...
	or '!cody.feedme' in trigger:
		...
		#try:
			...
			for item in feedData.keys():
				...
				elif feedData[item]['content'] not in checkfile:
					newData.append(feedData[item]['content'])
...
def codyFishbot(SESSION_DATA, trigger):
	
	if '!cody.fish' in trigger \
	and trigger in SESSION_DATA['MESSAGE'] \
	and SESSION_DATA['CODY_FISHBOT']:
		SESSION_DATA['CODY_FISHBOT'] = False
		SESSION_DATA['IRC'].send('PRIVMSG '+SESSION_DATA['CHANNEL']+' :Fishbot mode is now turned off.\r\n')

	elif '!cody.fish' in trigger \
	and trigger in SESSION_DATA['MESSAGE'] \
	and not SESSION_DATA['CODY_FISHBOT']:
		SESSION_DATA['CODY_FISHBOT'] = True
		SESSION_DATA['IRC'].send('PRIVMSG '+SESSION_DATA['CHANNEL']+' :Fishbot mode is now turned on.\r\n')

	for key in SESSION_DATA['CODY_FISHBOT_REPLIES'].keys():
		if key.lower() in SESSION_DATA['MESSAGE'].lower()\
		and SESSION_DATA['CODY_FISHBOT'] \
		and not '!cody.fish' in SESSION_DATA['MESSAGE']:
			SESSION_DATA['IRC'].send('PRIVMSG '+SESSION_DATA['CHANNEL']+' :'+SESSION_DATA['CODY_FISHBOT_REPLIES'][key]+'.\r\n')