#!/usr/bin/python
#-*- coding: utf8 -*-
import time, os, sys, stat, inspect, random, re, smtplib, socket, py_compile, datetime, feedparser, socket, select, json
...
class Cody():  
	...
	def __init__(self):
		...
		self.ADMIN_FUNCTIONS 			= 	{	 '!cody.join':self.joinChannel,			'!cody.part':self.partChannel, 			'!cody.reload':self.codyReload, 	
												 ...
												 '!cody.portscan':self.portscan, 		'!cody.diag':self.codyDiag,				'!cody.nick':self.codyNick, #'!cody.checker':self.codyMonitor,
												 '!cody.ghost':self.codyGhost,			'!cody.starbound':self.codyStarbound }
		...
		self.PUBLIC_FUNCTIONS			=	{	 '!cody.python':self.codyPython,		' cody':self.hiCody,					'!cody.version':self.codyVersion, 				 
												 ...
												 '!cody.action':self.codyAction, 		'!cody.google':self.codyGoogle }
	
		self.HOME_FUNCTIONS 			=	{	 'http':self.getTitle    } 
	
		self.PASSIVE_FUNCTIONS			=   {	 'codyGreeting':self.codyGreeting, 		'codyFileUpdates':self.codyFileUpdates, 'codyFeeds':self.codyFeeds}#, 'codyMonitor':self.codyMonitor}
	...
	def resolve(self, trigger):
		...
		def iptocountry(ip):
			...
			with open("resources/countrycodes_JSON.db","r") as countryCodeFile:
	...
	def codyDiag(self, trigger):
		...
		if diskStats and cpuStats:
			...
			self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+ " :An error occured while processing your request\r\n")


	def codyGoogle(self, trigger):

		if len(self.MSG_BODY) > len(trigger + '\r\n'):

			inputstring = self.MSG_BODY.lower().split(" ")
			query_msg	= " ".join(inputstring[1:]).strip("\r\n")

			queryinput          = query_msg
			querysanitation     = queryinput.replace(" ", "%20")
			website             = "https://www.google.com/#q="
			fullurl             = website+querysanitation 

			self.IRC.send("PRIVMSG "+self.MSG_CHANNEL+ " :" + fullurl + "\r\n")
	...
	def codyStarbound(self, trigger):
		...
		if len(self.MSG_BODY) > len(trigger + '\r\n'):
			...
			def StartStarbound():
 				try:
					cmd 	= "su starbound -c 'cd /home/starbound/server/linux64; ./run.sh'" #run starbound with starbound account
					pro 	= Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True)
					output 	= pro.communicate()
					splitit	= str(output[0])
					sb_PID  = splitit.split()
				except:
					pass
			...
			def StopStarbound():
				try:
					...
					for line in output.split('\n'):
					    if 'starbound_server' in line:
					        pid = line.split(" ")[2] #the process id from the ps aux command
			...
			if 'start' in command:
				...
				self.IRC.send("PRIVMSG "+self.HOME_CHANNEL+ " :Starbound server started\r\n")

			if 'stop' in command or 'terminate' in command:
				
				StopStarbound()
				
				self.IRC.send("PRIVMSG "+self.HOME_CHANNEL+ " :Starbound server stopped\r\n")

			if 'reload' in command or 'reboot' in command:
			
				StopStarbound()
				StartStarbound()

				self.IRC.send("PRIVMSG "+self.HOME_CHANNEL+ " :Starbound server restarted\r\n")			

		else:
			self.IRC.send("PRIVMSG "+self.HOME_CHANNEL+ " :Usage: !cody.starbound [start/stop/reload]\r\n")
...
#useful command: du -x /|sort -rnb|more
