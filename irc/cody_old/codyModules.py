...
def hiCody(SESSION_DATA, trigger):  
	...
	proximityMaximum = 3
	message 	     = SESSION_DATA['MESSAGE'].lower()
	codyPos  		 = message.find('cody')
	codesPos  		 = message.find('codes')
	nickPos			 = codyPos if codyPos != -1 else codesPos

	for reply_trigger in CODY_REPLY_TRIGGERS:

		triggerPos = message.find(reply_trigger)
		
		if reply_trigger in message \
		and ',' not in message \
		or reply_trigger[:-1] + ', ' in message:
			
			if triggerPos < nickPos\
			and triggerPos + len(reply_trigger) + proximityMaximum >= nickPos:
