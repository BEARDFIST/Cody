...
#!hiCody	
	...
	hiCodyTriggers = [ 'hi', 'hello', 'sup', 'yo', 'hey', 'hola', 'you suck', 'i love you' ]


	if 'cody' in MESSAGE.lower():
		i = 0
		proximityMaximum = 5
		while i < len(hiCodyTriggers) - 1:
			if hiCodyTriggers[i] in MESSAGE.lower() 																				\
			and MESSAGE.lower().find(hiCodyTriggers[i]) < MESSAGE.lower().find('cody')												\
			and MESSAGE.lower().find(hiCodyTriggers[i]) + len(hiCodyTriggers[i]) + proximityMaximum >= MESSAGE.lower().find('cody') \
			and connected == True:
...
#!cody.bug
	...
	and		connected 			== True 			:
		...
		codyBug 					= codyBug[1]
		addBug(codyBug)				#adds the reported bug to Cody/userFeedback/bugReports.txt
