...
class Cody():  
	...
	def kickstarter(self, trigger):
		...
		def IntCutter(fullstring, starter, ender):
			...
			print stringAndSuffix
			stringpart		= str(stringAndSuffix[2]).partition(ender)
			print stringpart
			stringsplit		= stringpart[0].split(".")
			print stringsplit
			#stringdecode	= stringsplit[0].decode('utf8') REMOVED DUE TO ENCODING FIX IN TOP OF FILE
			#stringencode	= stringdecode.encode('ascii', 'ignore')
				...
				if len(projectsplit) == 2:
					...
					for line in kickfile.read().splitlines():
						if len(line) != 0:
							...
							#if projectsplit[1].lower() == projectandname[1].lower():
							if str(one) == str(two):
	...
	def name(self, trigger):
		...
		while length >= i: 
			...
			#if "1" in str(i) or "3" in str(i) or "5" in str(i) or "7" in str(i):
			if i%2 != 0:
				word += vowels[intV]
